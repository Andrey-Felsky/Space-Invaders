#include "logic.h"
#include "../mapa/mapa.h" // Para ALTURA_MAPA, LARGURA_MAPA
#include "../utils/constants.h" // Para ENEMY_EXPLOSION_FREQ, etc.
#include <windows.h> // Para o Beep()
#include <cstdlib> // Para o rand()
#include <algorithm> // Para o min, remove_if

extern GameMode currentGameMode;
chrono::high_resolution_clock::time_point lastEnemyShotTime;

Barrier barriers[NUM_BARRIERS];

// Atualiza modelo do tiro do jogador
void updatePlayerBullets() {
    for (int i = 0; i < 2; ++i) {
        for (auto& bullet : players[i].bullets) {
            bullet.second--; // Move tiro pra cima
        }
        players[i].bullets.erase(remove_if(players[i].bullets.begin(), players[i].bullets.end(),
...
void updateTiroInimigo() {
    ...
    // 2. Dispara tiro do inimigo
    if (!tiroInimigoAtivo && (now - lastEnemyShotTime >= ENEMY_SHOT_INTERVAL)) {
        // encontra novo inimigo pra gerar o tiro
        vector<int> activeEnemiesIndices;
        ...
        if (!activeEnemiesIndices.empty()) {
            ...
            tiroInimigoY = inimigos[shooterIndex][1] + 1; // tiro inicia abaixo do atirador
            tiroInimigoAtivo = true;
            lastEnemyShotTime = now; // atualiza tempo de tiro pra agora
        }
    }
}
void updateItemDrop() {
...
// Função auxiliar para verificar colisões com um jogador, permitindo diferentes tamanhos de hitbox.
// hitboxWidth deve ser um número ímpar (1, 3, 5, etc.).
bool checkPlayerCollision(int objectX, int objectY, const Player& player, int hitboxWidth) {
    if (player.vidas <= 0) {
        return false; // caso colidir com player ja morto
    }

    // Verifique se o objeto esta acima do jogador
    if (objectY != ALTURA_MAPA - 1) {
        return false;
    }

    // Calcula limite horizontal pra colisao
