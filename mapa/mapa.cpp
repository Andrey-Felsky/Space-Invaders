...
void render(const Player players[2], float tempo)
{
    ...
    ssTempo << fixed << setprecision(1) << tempo << "s"; // tempo fica fixo em um local de exibição
    ...
    // Desenha os tiros dos jogadores
    for (int i = 0; i < 2; ++i) {
    ...
    for (int i = 0; i < numPlayersToRender; ++i) {
        if (players[i].vidas > 0) { //renderiza apenas se estiver vivo
    ...
    // Renderiza a explosao do jogador
    for (int i = 0; i < 2; ++i) {
    ...
    for (int y = 0; y < ALTURA_MAPA; y++)
    {
        ...
        for (int x = 0; x < LARGURA_MAPA; x++)
        {
            ...
            // verifica se o caracter parte da nave do jogador
            for (int i = 0; i < 2; ++i) {
